...
# ==============================================================================
# DETECTION LAN/WAN ІНТЕРФЕЙСІВ
# ==============================================================================
# Не парсимо netplan: він ненадійний з кількох причин:
#   • cloud-init на Ubuntu Server залишає 50-cloud-init.yaml зі застарілим eth0
#   • кілька .yaml файлів у /etc/netplan/ конфліктують — find | head -1 повертає
#     перший за filesystem inode, не за алфавітом
#   • синтаксис різниться між версіями v3/v4/v5 (optional:true маркер додано в v5)
#   • на v3-серверах WAN був першим, на v5 — другим (порядок різний)
#
# ip-команди читають актуальний стан ядра напряму:
#   • LAN: інтерфейс що має IP 10.10.1.1 (hardcoded LAN gateway MineVPN)
#   • WAN: інтерфейс дефолтного route (виключаючи VPN tun/wg)
#
# Установлює глобальні змінні DETECTED_LAN, DETECTED_WAN. Якщо щось не знайшлось —
# залишаються порожніми; викликач має зробити log_warn і пропустити операцію.
detect_interfaces() {
    DETECTED_LAN=$(ip -4 addr show 2>/dev/null | grep "10\.10\.1\.1/" | awk '{print $NF}')
...
STEOF
    ...
    # === 4. Kill Switch ===
    # Detection через detect_interfaces() — ip-команди, не netplan parsing.
    # Причина: netplan parsing ломався на серверах де в /etc/netplan/ є backup-фолдери
    # (find -recursive заходив у backup_*/00-default.yaml з eth0) або cloud-init файли.
    detect_interfaces
    LAN_IF="$DETECTED_LAN"
    WAN_IF="$DETECTED_WAN"
...
APACHEEOF
    ...
    # HC daemon (vpn-healthcheck.sh::load_interfaces) використовує цей файл як primary source
    # для WAN/LAN. Без нього fallback йде на ip-команди — працює, але логує WARN.
    #
    # РЕВАЛІДАЦІЯ: якщо файл вже є, але WAN/LAN в ньому вказують на інтерфейси
    # яких немає в системі — вважаємо файл хибним і перестворюємо. Це фіксить ситуацію коли
    # попереднє оновлення поклало невірний WAN в файл (напр. eth0 коли реально ens33).
    need_recreate_minevpn_conf=0
    if [ ! -f /etc/minevpn.conf ]; then
        need_recreate_minevpn_conf=1
    else
        existing_wan=$(grep "^WAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
        existing_lan=$(grep "^LAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
        if [ -z "$existing_wan" ] || ! ip link show "$existing_wan" >/dev/null 2>&1; then
            log_warn "/etc/minevpn.conf містить хибний WAN='$existing_wan' (інтерфейс не існує) — перестворюю"
            need_recreate_minevpn_conf=1
        elif [ -z "$existing_lan" ] || ! ip link show "$existing_lan" >/dev/null 2>&1; then
            log_warn "/etc/minevpn.conf містить хибний LAN='$existing_lan' (інтерфейс не існує) — перестворюю"
            need_recreate_minevpn_conf=1
        fi
    fi

    if [ "$need_recreate_minevpn_conf" = "1" ]; then
        # detect_interfaces могла бути викликана в блоці 4 — викликаємо знову для свіжих даних
        # (sys state міг змінитися між блоками, хоча очікуємо їдентичний результат).
        detect_interfaces
...
EOF
        log_info "/etc/minevpn.conf створено/оновлено (WAN=${DETECTED_WAN:-unknown}, LAN=${DETECTED_LAN:-unknown})"
    fi
...
# ==============================================================================
# /etc/minevpn.conf РЕВАЛІДАЦІЯ (виконується завжди)
# ==============================================================================
# HC daemon (vpn-healthcheck.sh::load_interfaces) використовує цей файл як primary source
# для WAN/LAN. Якщо файл містить хибні значення (напр. WAN=eth0 коли в системі ens33),
# HC daemon робить нескінченні спроби відновити Kill Switch який також налаштований невірно.
#
# Цей блок:
#   1. Перевіряє що інтерфейси з minevpn.conf реально існують у системі (ip link show)
#   2. Якщо хоч один не існує — перестворює файл через detect_interfaces (ip-команди)
#   3. Якщо файла немає взагалі — створює його
#
# Виконується ПОЗА блоком v5-міграції щоб виправляти існуючі сервери з багом —
# міграція v5 не повторюється для серверів які вже на v5 рівні.
minevpn_conf_recreate=0
if [ ! -f /etc/minevpn.conf ]; then
    minevpn_conf_recreate=1
else
    existing_wan=$(grep "^WAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
    existing_lan=$(grep "^LAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
    if [ -z "$existing_wan" ] || [ "$existing_wan" = "unknown" ] || ! ip link show "$existing_wan" >/dev/null 2>&1; then
        log_warn "/etc/minevpn.conf містить хибний WAN='$existing_wan' (інтерфейс не існує в системі) — перестворюю"
        minevpn_conf_recreate=1
    elif [ -z "$existing_lan" ] || [ "$existing_lan" = "unknown" ] || ! ip link show "$existing_lan" >/dev/null 2>&1; then
        log_warn "/etc/minevpn.conf містить хибний LAN='$existing_lan' (інтерфейс не існує в системі) — перестворюю"
        minevpn_conf_recreate=1
    fi
fi

if [ "$minevpn_conf_recreate" = "1" ]; then
    detect_interfaces
    cat > /etc/minevpn.conf << EOF
VERSION=$SCRIPT_VERSION
DATE=$(date '+%Y-%m-%d %H:%M:%S')
...
LAN=${DETECTED_LAN:-unknown}
EOF
    log_info "/etc/minevpn.conf оновлено (WAN=${DETECTED_WAN:-unknown}, LAN=${DETECTED_LAN:-unknown})"
    # Перезапуск HC daemon щоб перечитав новий minevpn.conf (без чекання 5 хв до load_interfaces)
    if systemctl is-active --quiet vpn-healthcheck.service 2>/dev/null; then
        systemctl restart vpn-healthcheck.service 2>/dev/null && log_info "HC daemon перезапущено для перечитання minevpn.conf"
    fi
fi

# ==============================================================================
# KILL SWITCH РЕВАЛІДАЦІЯ (виконується завжди)
# ==============================================================================
# Перевіряємо що в FORWARD chain є наші Kill Switch правила. Якщо detection при міграції
# v3→v5 провалився (старий netplan-баг) — правил немає, і HC daemon в loop'і
# намагається їх відновити з rules.v4 (де їх теж немає). Цей блок виправляє ситуацію.
#
# Перевіряє ті ж 3 правила що й HC daemon::check_iptables:
#   1. FORWARD policy = DROP
#   2. NAT POSTROUTING -o tun0 -s 10.10.1.0/20 MASQUERADE
#   3. FORWARD -i $LAN -o tun0 ACCEPT
#
# Якщо хоч одне відсутнє І в нас є правильні WAN/LAN з detect_interfaces — налаштовуємо.
if [ -f /etc/minevpn.conf ]; then
    ks_lan=$(grep "^LAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
    ks_wan=$(grep "^WAN=" /etc/minevpn.conf 2>/dev/null | cut -d= -f2)
    if [ -n "$ks_lan" ] && [ -n "$ks_wan" ] && [ "$ks_lan" != "unknown" ] && [ "$ks_wan" != "unknown" ]; then
        ...
        [ "$ks_bad" -eq 0 ] && ! iptables -t nat -C POSTROUTING -o tun0 -s 10.10.1.0/20 -j MASQUERADE 2>/dev/null && ks_bad=1
