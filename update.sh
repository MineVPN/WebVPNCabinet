...
EOF
    ...
    # Добавлено Автовосстановление VPN-туннеля.
    # -----------------------------------------------------------------
...
EOF
    ...
    if [ "$CURRENT_VERSION" -lt 4 ]; then
        FILE_PATH="/usr/local/bin/vpn-healthcheck.sh"
        ...
        echo "Выполнение модификаций в файле $FILE_PATH..."


        sudo sed -i \
            -e '/#log "Автоподнятие интерфейса отключено в настройках. Выход."/a\exit 1' \
            -e '/# В любом случае выходим из скрипта, так как интерфейса нет и дальнейшие проверки бессмысленны./{n;d;}' \
            "$FILE_PATH"

        if [ $? -eq 0 ]; then
            echo "✅ Модификации успешно применены:"
            echo "  - Строка 'exit 1' вставлена после '#log \"Автоподнятие интерфейса...\"'."
            echo "  - Строка 'exit 1' удалена после '# В любом случае выходим из скрипта...'"
        else
            echo "❌ Ошибка при выполнении sed. Проверьте права доступа или содержимое файла."
        fi
