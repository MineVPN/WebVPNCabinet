<?php
// ==============================================================================
// MINE SERVER - Управление WireGuard
// ==============================================================================
    ...
    if (isset($_FILES["config_file"])) {
        if (!empty($_FILES["config_file"]["name"])) {
            ...
            if (in_array($file_extension, $allowed_extensions)) {
                ...
                echo "<script>Notice('Разрешены только файлы с расширением .conf');</script>";
            }
        }
    }
}
    ...
    <div class="glassmorphism rounded-2xl p-6 flex flex-col">
        ...
        <div class="space-y-4 text-slate-300 flex-grow">
            ...
            <div class="flex justify-between items-center">
                <span class="font-medium">IP-адрес сервера:</span>
        ...
        <form id="vpn-control-form" method="post" class="mt-8">
            <?php if ($type == "wireguard"): ?>
                <button type="submit" name="wireguard_start" id="wireguard-start-btn" 
                ...
                <button type="submit" name="wireguard_stop" id="wireguard-stop-btn" 
            ...
            <?php else: ?>
                <button disabled class="w-full bg-slate-700 text-slate-500 font-bold py-3 rounded-lg cursor-not-allowed">
                    <?= $type == "openvpn" ? "Активен OpenVPN (не WireGuard)" : "Конфигурация не загружена" ?>
                </button>
            <?php endif; ?>
        </form>
    </div>
            ...
            <div class="flex-grow">
                <label id="drop-zone" for="config_file" 
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="text-xs text-slate-500">только *.conf (WireGuard)</p>
                    </div>
...
<div class="glassmorphism rounded-2xl p-6 mt-8">
    <h3 class="text-xl font-bold text-white mb-4">О WireGuard</h3>
    <div class="text-slate-400 space-y-2">
        <p>WireGuard — современный VPN-протокол, отличающийся высокой скоростью и простотой настройки.</p>
        <p><strong class="text-slate-300">Преимущества:</strong></p>
        <ul class="list-disc list-inside ml-4 space-y-1">
            <li>Минимальная задержка (идеально для игр)</li>
            <li>Высокая скорость передачи данных</li>
            <li>Современная криптография</li>
            <li>Компактный код (меньше уязвимостей)</li>
        </ul>
    </div>
</div>
...
<script>
    ...
    // Обработка формы управления VPN
    ...
    if (vpnForm) {
        vpnForm.addEventListener('submit', function(e) {
            ...
            const startBtn = document.getElementById('wireguard-start-btn');
            const stopBtn = document.getElementById('wireguard-stop-btn');
            const originalStartText = startBtn ? startBtn.textContent : '';
            const originalStopText = stopBtn ? stopBtn.textContent : '';
            ...
            if (submitter && submitter.name === 'wireguard_start') {
                if (startBtn) startBtn.textContent = 'Запускается...';
            } else if (submitter) {
                if (stopBtn) stopBtn.textContent = 'Останавливается...';
            }
